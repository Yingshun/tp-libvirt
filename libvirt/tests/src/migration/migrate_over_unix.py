...
def run(test, params, env):
    ...
    def update_disk(vm, params):
        ...
        if storage_type == "nfs":
            ...
            for idx in range(2, disk_num+1):
                ...
                logging.debug("status: {}, stdout: {}".format(status, stdout))
                if status:
                    test.fail("Failed to create img on remote: cmd: {} "
                              "status: {}, stdout: {}"
    ...
    try:
        ...
        if migrate_again:
            ...
            time.sleep(10)
            if params.get("migrate_again_clear_func") == "yes":
                func_name = None
            if params.get("migrate_again_status_error") == "no":
